...
class Multiplayeronline(maingame): #still currently working on
    ...
    def MOgameplay(self):
        ...
        self.multiplayeronline = True
        self.firstplayerobject.draw(deck, 5)
        self.secondplayerobject.draw(deck, 5)
        self.firstplayerobject.showhand()
    ...
    def gamemanager(self):
        self.firstplayerobject.showhand()
        self.secondplayerobject.showhand()
    ...
    def main(self): #ADD USER GAMEPLAY BETWEEN SERVERS AND THEN TEST EVERYTHING
        run = True
        pygame.mixer.music.load("Menumusic_2.mp3")
        pygame.mixer.music.play(-1)
        ...
        while run:
            player2 = n.send(p)
            self.secondplayerobject = player2
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    run = False
                    pygame.quit()
            if self.firstplayerobject.won == True: 
                run = False
                Multiplayeronline.MOwinnerscreen(self.firstplayerobject)
            elif self.secondplayerobject.won == True:
                run = False
                Multiplayeronline.MOwinnerscreen(self.secondplayerobject)
            else:
                Multiplayeronline.MOgameplay(self)
            pygame.display.update()
